...
var errnoMsg = map[Errno]string{
	...
	ERROR_DUP_NAME:                                             `You were not connected because a duplicate name exists on the network. If joining a domain, go to System in Control Panel to change the computer name and try again. If joining a workgroup, choose another workgroup name.`,
	...
	ERROR_EXE_MACHINE_TYPE_MISMATCH:                            `This version of %1 is not compatible with the version of Windows you're running. Check your computer's system information and then contact the software publisher.`,
	...
	ERROR_FORMS_AUTH_REQUIRED:                                  `Access Denied. Before opening files in this location, you must first add the web site to your trusted sites list, browse to the web site, and select the option to login automatically.`,
	...
	ERROR_VIRUS_DELETED:                                        `This file contains a virus or potentially unwanted software and cannot be opened. Due to the nature of this virus or potentially unwanted software, the file has been removed from this location.`,
	...
	ERROR_INCOMPATIBLE_WITH_GLOBAL_SHORT_NAME_REGISTRY_SETTING: `Short name settings may not be changed on this volume due to the global registry setting.`,
	ERROR_SHORT_NAMES_NOT_ENABLED_ON_VOLUME:                    `Short names are not enabled on this volume.`,
	ERROR_SECURITY_STREAM_IS_INCONSISTENT:                      `The security stream for the given volume is in an inconsistent state. Please run CHKDSK on the volume.`,
	...
	ERROR_INTERMIXED_KERNEL_EA_OPERATION:                       `An attempt was made to modify both a KERNEL and normal Extended Attribute (EA) in the same operation.`,
	ERROR_FILE_LEVEL_TRIM_NOT_SUPPORTED:                        `Device does not support file-level TRIM.`,
	ERROR_OFFSET_ALIGNMENT_VIOLATION:                           `The command specified a data offset that does not align to the device's granularity/alignment.`,
	...
	ERROR_BAD_DEVICE_PATH:                                      `An attempt was made to send down the command via an invalid path to the target device.`,
	ERROR_TOO_MANY_DESCRIPTORS:                                 `The command specified a number of descriptors that exceeded the maximum supported by the device.`,
	...
	ERROR_LOST_WRITEBEHIND_DATA_NETWORK_SERVER_ERROR:              `{Delayed Write Failed} Windows was unable to save all the data for the file %hs; the data has been lost. This error was returned by the server on which the file exists. Please try to save this file elsewhere.`,
	...
	ERROR_IMAGE_AT_DIFFERENT_BASE:                                 `An image file was mapped at a different address from the one specified in the image file but fixups will still be automatically performed on the image.`,
	...
	ERROR_LAST_ADMIN:                         `This operation is disallowed as it could result in an administration account being disabled, deleted or unable to log on.`,
	...
	ERROR_CLUSTER_NODE_DRAIN_IN_PROGRESS:                                     `A node drain is already in progress.  This value was also named ERROR_CLUSTER_NODE_EVACUATION_IN_PROGRESS`,
	ERROR_CLUSTER_DISK_NOT_CONNECTED:                                         `Clustered storage is not connected to the node.`,
	ERROR_DISK_NOT_CSV_CAPABLE:                                               `The disk is not configured in a way to be used with CSV. CSV disks must have at least one partition that is formatted with NTFS.`,
	...
	DNS_ERROR_NSEC3_INCOMPATIBLE_WITH_RSA_SHA1:      `NSEC3 is not compatible with the RSA-SHA-1 algorithm. Choose a different algorithm or use NSEC.  This value was also named DNS_ERROR_INVALID_NSEC3_PARAMETERS`,
